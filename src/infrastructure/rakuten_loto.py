...
class RakutenLotoClient:
    ...
    - HTML取得とパースを分離
    - バリデーション・詳細ログ付きで堅牢化
    - 外部にはドメインモデル(Loto6Result/Loto7Result)を返す
    """
    LOTO_URLS = {
    ...
    def fetch_latest_result(self, lottery_type: str) -> Loto6Result | Loto7Result:
        """
        指定ロト種別の最新抽選結果を取得し、ドメインモデルで返す。
        Args:
            lottery_type (str): 'loto6' or 'loto7'
        Returns:
            Loto6Result | Loto7Result: 取得結果
        Raises:
            Exception: パース失敗時
        """
        ...
        try:
            ...
            raw_result = self._parse_html(lottery_type, html, url)
            raw_result.validate()
            logger.info(f"[RakutenLotoClient] 取得成功: type={lottery_type} no={raw_result.draw_no} date={raw_result.draw_date}")
            return self._to_domain_model(raw_result)
        except Exception as e:
            logger.error(f"[RakutenLotoClient] パース失敗: type={lottery_type} url={url} error={e}", exc_info=True)
            raise
    ...
    def _parse_html(self, lottery_type: str, html: str, url: str) -> _LotoResult:
        """
        HTMLから抽選結果を厳密にパースする。
    ...
    def _to_domain_model(self, raw: _LotoResult) -> Loto6Result | Loto7Result:
        """
        内部LotoResultからドメインモデルへ変換。
        Returns: Loto6Result or Loto7Result
        """
        if raw.lottery_type == "loto6":
            return Loto6Result(
                draw_number=raw.draw_no,
                draw_date=raw.draw_date,
                numbers=raw.main_numbers,
                bonus=raw.bonus_numbers[0],
            )
        elif raw.lottery_type == "loto7":
            return Loto7Result(
                draw_number=raw.draw_no,
                draw_date=raw.draw_date,
                numbers=raw.main_numbers,
                bonus1=raw.bonus_numbers[0],
                bonus2=raw.bonus_numbers[1],
            )
        else:
            raise ValueError(f"unsupported lottery_type: {raw.lottery_type}")
